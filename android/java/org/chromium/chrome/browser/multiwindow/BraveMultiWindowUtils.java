...
public class BraveMultiWindowUtils extends MultiWindowUtils {
    ...
    public static boolean shouldShowManageWindowsMenu() {
        return shouldEnableWindows() && MultiWindowUtils.shouldShowManageWindowsMenu();
    }

    @Override
    public boolean isOpenInOtherWindowSupported(Activity activity) {
        return shouldEnableWindows() && super.isOpenInOtherWindowSupported(activity);
    }

    @Override
    public boolean isMoveToOtherWindowSupported(
            Activity activity, TabModelSelector tabModelSelector) {
        return shouldEnableWindows()
                && super.isMoveToOtherWindowSupported(activity, tabModelSelector);
    }

    @Override
    public boolean canEnterMultiWindowMode(Activity activity) {
        return shouldEnableWindows() && super.canEnterMultiWindowMode(activity);
    }

    public static boolean shouldEnableWindows() {
        return ChromeSharedPreferences.getInstance()
                .readBoolean(BravePreferenceKeys.ENABLE_WINDOWS, false);
    }

    public static void updateEnableWindows(boolean isEnable) {
        ChromeSharedPreferences.getInstance()
                .writeBoolean(BravePreferenceKeys.ENABLE_WINDOWS, isEnable);
    }

    public static boolean isCheckUpgradeEnableWindows() {
        return ChromeSharedPreferences.getInstance()
                .readBoolean(BravePreferenceKeys.ENABLE_WINDOWS_UPGRADE, false);
    }

    public static void setCheckUpgradeEnableWindows(boolean isUpgradeCheck) {
        ChromeSharedPreferences.getInstance()
                .writeBoolean(BravePreferenceKeys.ENABLE_WINDOWS_UPGRADE, isUpgradeCheck);
    }
